...
object StreamDataCalc {
  def main(args: Array[String]): Unit = {
    ...
    gmvStream.addSink(new GmvNeo4jSink("aliyunecs",6380,"gmv"))


    //2:统计实时销量TopN的商品品类
    val topNStream = filterStreram.map(tup=>(tup._3,tup._1))//获取商品品类和购买的商品数量
      .keyBy(tup=>tup._1)//根据商品品类分组
      .timeWindow(Time.seconds(1))//设置时间窗口为1秒
      .sum(1)//指定tuple中的第二列，也就是商品数量

    topNStream.addSink(new TopnNeo4jSink("aliyunecs",6380,"goods_type"))
